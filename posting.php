...
if ($submit || $preview || $refresh)
{
	...
	/* custom spam prevention ~ Eric M. Feb 23rd */
	if ($user->data['posts'] <= 10 && substr_count(utf8_normalize_nfc(request_var('message', '', true)), '[/url]') ) {
		$sql = 'SELECT count(*) as postCount FROM rpg_content WHERE author_id = '.(int) $user->data['user_id'];
		$postResult = $db->sql_query($sql);
		$stats = $db->sql_fetchrow($postResult);
		$db->sql_freeresult($postResult);

		if ($stats['postCount'] < 10) {
			trigger_error('Whoa there!  We haven\'t seen very much of you before!<br/><br/>Participate in the community a bit more before trying to post links.<br/><br/><a class="button" href="javascript:history.back();">&laquo; Go Back</a>');
		}
	...
	// Custom spam prevention - Eric M, 8/12/2008
	$sql = 'SELECT topic_id,post_id 
		FROM ' . POSTS_TABLE . '
		WHERE post_checksum = \'' . $message_md5.'\'';
	$result = $db->sql_query($sql);
	$post_row = $db->sql_fetchrow($result);
	$db->sql_freeresult($result);
	
	if (count($post_row) >= 2 && $post_row['post_id'] != $post_id) {
	
		$other_post_url = $phpbb_root_path."viewtopic.$phpEx?p=".$post_row['post_id']."#p".$post_row['post_id'];

		$error[] = "Someone else has <a href=\"".$other_post_url."\">already said that</a>! You can't make the same post twice...<br /><br />
				If you write a longer post, it is less likely for someone to have written the same thing before.";
	}
	// End custom spam prevention

	// Custom double post prevention, Eric M, 12/1/2008
	// If the user can not ignore flood limit, do a check for double post
	if (!$auth->acl_get('u_ignoreflood')) {
		$sql = 'SELECT topic_last_poster_id,topic_last_post_id 
			FROM ' . TOPICS_TABLE . '
			WHERE topic_id = \'' . $topic_id .'\'';
		$result = $db->sql_query($sql);
		while ($topic_poster_row = $db->sql_fetchrow($result)) {
			$topic_last_poster_id = $topic_poster_row['topic_last_poster_id'];
			$topic_last_post_id = $topic_poster_row['topic_last_post_id'];
		}
		$db->sql_freeresult($result);
		
		if (@$topic_last_poster_id == $user->data['user_id'] && $mode != 'edit') {
			...
			$result = $db->sql_query($sql);
			$non_author_posts = (int) $db->sql_fetchfield('num_posts');
			$db->sql_freeresult($result);

			if ($non_author_post > 0) {
				$edit_last_post_url = $phpbb_root_path."posting.$phpEx?mode=edit&f=".$post_data['forum_id']."&p=".$topic_last_post_id;
				$error[] = "You can't reply twice in a row to the same topic, that's called <em>double posting</em>.  Wait for someone else to reply or <a href=\"".$edit_last_post_url."\">edit your last post</a>.";
			}
		}
	}
	// End custom double post prevention
