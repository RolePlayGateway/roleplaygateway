...
if ($roleplay = $db->sql_fetchrow($result)) {
  ...
  $sql = 'SELECT count(DISTINCT content_id) as total FROM rpg_arc_content WHERE arc_id = '. (int) $arc['id'];
  $tagResult = $db->sql_query($sql);
  $stats = $db->sql_fetchrow($tagResult);

  if ($stats['total'] >= 20) {
    award_medal_from(0, 21, $user->data['user_id'], 'You added 20 posts to a single bundle!', time());
  }
  
  if ($stats['total'] >= 100) {
    award_medal_from(0, 34, $user->data['user_id'], 'You added 100 posts to a single bundle!', time());
  }
