...
if ($roleplay = $db->sql_fetchrow($result)) {
  ...
  $sql = 'SELECT id, name, description, slug, creator FROM rpg_arcs
    WHERE roleplay_id = '.(int) $roleplay['id'].'
      ORDER BY created DESC';
  $arcResult = $db->sql_query($sql);
  while ($arc = $db->sql_fetchrow($arcResult)) {

    $sql = 'SELECT username FROM gateway_users
              WHERE user_id = '.(int) $arc['creator'];
    ...
    $template->assign_block_vars('arcs', array(
      ...
      'NAME'            => $arc['name'],
      'SLUG'            => $arc['slug'],
      'DESCRIPTION'     => generate_text_for_display($arc['description'], $arc['description_uid'], $arc['description_bitfield'], 7),
      'CREATOR_USERNAME'=> get_username_string('full', @$arc['creator'], @$owner['username']),
  ...
  page_header('Arcs | ' . $roleplay['title'] . ' | ' . $config['sitename']);
