 ...
 * @package phpBB3
 * @version $Id: weekly_posts.php 1097 2008-12-12 08:16:43Z xhotshotx $
...
for($i = 6; $i > -1; $i--)
{
    $begin = gmmktime(0, 0, 0, date("m"), date("d") - (($i + 1) * 7), date("Y"));
    $end = gmmktime(0, 0, 0, date("m"), date("d") - ($i * 7), date("Y"));
...
// add something at the end of the string to grab
$sql .= '**';

// MySQL doesn't like ending commas, so we remove it.
$sql = str_replace(',**', '', $sql);
...
$graph->title->Set('Posts Per Week');
