		...
		for(var i=0; i<messageNodes.length; i++) {
			...
			try {
				...
				if (typeof(characterNode) == 'undefined') {
					...
					if (messageText.indexOf('/describe') == 0) {
...
ajaxChat.getChatListMessageString = function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip, characterName, characterID, roleplayID, characterURL, roleplayURL) {
	...
	} else if (messageText.indexOf('/describe') == 0) {
		userClass += ' action';
		colon = ' writes, ';	
	} else {
		messageType = 'Speech';
...
// 		} elseif (roleplayURL.length() > 0) {
			...
			icMessage += '<div style="padding-left:180px; clear:none;">';
			...
			if ((channelID > 0 && channelID < channelLimit) && (messageText.indexOf('/describe') != 0)) {
				rowClass += ' ooc';
			}
