	...
	function getMessageCondition() {
		...
		if ($this->getRequestVar('lastID') > 0) {
			...
			$condition .= 	'
				...
				dateTime >= "'.date('Y-m-d H:i:s', $this->getChannelEnterTimeStamp() - 1800).'"
		...
		switch($tag) {
			...
			case 'PLACES_SELECTION':
				...
				$places_selection = '<div id="placeCurrent" class="ui-state-default" onclick="openModal(\'placesControlContainer\');" id="placeIndicator">'.$placeName.'</div><div id="placesControlContainer" style="display:none;">
					...
					foreach ($places as $place) {
						...
						<img style="float:left;" src="http://www.roleplaygateway.com/images/places_thumbnail.php?place_id='.$place['id'].'" /><strong>'.$place['name'].'</strong><p>'. $place['synopsis'] .'</p></div>';
					}
			...
			case 'ROLEPLAY_SELECTION':
				...
				if ($user->data['user_id'] > 1) {
					...
					$sql = 'SELECT title,id,description FROM rpg_roleplays where id = 1 LIMIT 1';
					$result = $db->sql_query($sql);
					while ($row = $db->sql_fetchrow($result)) {
					...
					$sql = 'SELECT title,id,description FROM rpg_roleplays WHERE id IN (SELECT roleplay_id FROM rpg_characters WHERE owner = '.(int) $user->data['user_id'].' AND roleplay_id <> 1) AND status = "Open" LIMIT 10';
					$result = $db->sql_query($sql);
					while ($row = $db->sql_fetchrow($result)) {
						$roleplays[$row['id']] = $row;
		...
		if ((((substr($text,0,3) != "/ic")
			...
			&& ((substr($text,0,1) != "!") && (($this->getUserRole() != AJAX_CHAT_ADMIN)) )
				...
				if(!$this->isAllowedToWriteMessage()) {
					$this->insertChatBotMessage($this->getPrivateMessageID(),"[b]You cannot post because you are not logged in as a registered user. You should [url=http://www.roleplaygateway.com/ucp.php?mode=register]register[/url] or [url=http://www.roleplaygateway.com/ucp.php?mode=login]log in[/url].[/b]");
					return;
				}
