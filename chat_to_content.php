...
$sql = 'SELECT m.id,m.channel,m.userID,m.userName,m.dateTime,m.text FROM ajax_chat_messages m
			...
			WHERE m.id > (SELECT MAX(old_chat_id) - 100000 FROM rpg_content) AND m.channel <> 0 and m.userID <> 2147483647
...
while ($message = $db->sql_fetchrow($chat_result)) {
	...
	while ($place = $db->sql_fetchrow($places_result)) {
		...
		if (($message['userName'] == $user['username']) && ($textParts[0] != '/say')) {
		...
		if (!empty($character['id']) || $textParts[0] == '/say') {
