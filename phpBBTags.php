...
switch($mode){
	...
	case 'add_tag':
		...
		if(trim($tags) != '')
		{
			...
			$message = sprintf($user->lang['PBT_ADD_TAGS_DONE'], $tag_outcome['added'], $tag_outcome['dups']) . '<br /><br />' . sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $meta_info . '">', '</a>');
			meta_refresh(3, $meta_info);
		}
		else
		{
			$meta_info = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $forum_id . '&t=' . $topic_id .'#phpBBTags_input');
			$message = $user->lang['PBT_ADD_TAGS_NO_TAGS'] . '<br /><br />' . sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $meta_info . '">', '</a>');
		}
	...
	default:
		...
		if($search_tag == '')
		{
			$template->assign_vars(array(
				'S_TAG_SEARCH_ACTION'		=> append_sid("{$phpbb_root}phpBBTags.{$phpEx}", 'mode=search'),
				'S_SEARCH_STRING'			=> $search_tag,
				//'TAG_CLOUD'					=> get_tag_cloud(),
				)
			);		
			page_header($user->lang['PBT_SEARCH_PAGE_TITLE']);
					
			$template->set_filenames(array(
				'body' => 'phpBBTags_search.html')
			);	
		...
		else
		{
			...
			{	
				...
				$template->assign_vars(array(
					'PAGINATION'	=> generate_pagination(append_sid("{$phpbb_root_path}phpBBTags.$phpEx","tag=$search_tag"), $topics_count, $config['topics_per_page'], $start),
					'PAGE_NUMBER'	=> on_page($topics_count, $config['topics_per_page'], $start),
					'TOTAL_TOPICS'	=> ($s_display_active) ? false : (($topics_count == 1) ? $user->lang['PBT_NUM_TOPIC'] : sprintf($user->lang['PBT_NUM_TOPICS'], $topics_count)),
				...
				page_header($user->lang['PBT_TAGS_RESULT_TITLE']);
					
				$template->set_filenames(array(
					'body' => 'phpBBTags_search_results.html')
				);	
				
				page_footer();			
			}else{
			
				$meta_info = append_sid("{$phpbb_root_path}phpBBTags.$phpEx");
				$message = sprintf($user->lang['PBT_NO_RESULTS'], $search_tag) . '<br /><br />' . sprintf($user->lang['PBT_RETURN_TO_SEARCH'], '<a href="' . $meta_info . '">', '</a>');
