...
/*              if (characterNode) { 
            ...
            //if ((messageText.replace(/[^A-Z]/g, "").length) >= (messageText.replace(/[A-Z]/g, "").length)) {
            if ((messageText.replace(/[^A-Z]/g, "").length) >= 30) {
            ...
            */
                var cmd = '';
                           //Take out command if any
                if(messageText.substr(0,4) == '/msg')
                {
                       //Find index of second space i.e the one after the username
                       var cmdEnd = messageText.indexOf(' ', 5);
                       //Copy cammand and name for use later
                       cmd = messageText.slice(0,cmdEnd+1);
                       //Remove command from string
                       messageText = messageText.slice(cmdEnd,messageText.length);
                }
                //Convert to lower case
                messageText = messageText.toLowerCase();

                //Replace Qoute BBcode with "
                messageText = messageText.replace(/\[[\/]?quote\]([\w]?)\1/g,'"$1');
                                                                                                                                                   
                //Remove all remaining BBcode except the URL
                messageText = messageText.replace(/\[[\/]?(b|i|u|s|code|img|color(=\w+)?)\]/g,"");
                //messageText = this.stripBBCodeTags(messageText);   

                //Add . to end of last sentance if there is no ending char
                if( (messageText.charAt(messageText.length-1) != ".")&&(messageText.charAt(messageText.length-1) != "?")&&(messageText.charAt(messageText.length-1) != "!")&&(messageText.charAt(messageText.length-1) != "\""))
                {
                    messageText = messageText + '.';
                }        
                //Replace any more than 3 fullstops with one
                messageText = messageText.replace(/(.)\1{3,}/g,"$1");
                
                //Split when there is a . ? ! or "
                sentences = messageText.split(/[.?!"\n]/);
                
                var joinedMessageText= '';
                var senLen = sentences.length - 1;
                var j;
                var lastIndex = -1;

                //For all the sentances
                for(j=0;j<senLen;j++)
                {
                    tempChar = 99999; // Larger than max post.
                    //Find the min of the next end of sentance
                    //Get location of next . if -1 none left so ignore
                    if(messageText.indexOf('.',lastIndex+1) != -1)
                        tempChar = Math.min(tempChar, messageText.indexOf('.',lastIndex+1));
                    ...
                    //Get location of next \n if -1 none left so ignore
                    if (messageText.indexOf('\n',lastIndex+1) != -1)
                        tempChar = Math.min(tempChar, messageText.indexOf('\n',lastIndex+1));
                    //Copy min of end of sentance to index
                    lastIndex=tempChar;
                    
                    //Get the char to use on end of sentance
                    endChar = messageText.charAt(lastIndex);
                    ...
                    if ((endChar  == '\n'))
                        ...
                        msg = msg.slice(0,1).toUpperCase() + msg.slice(1)+ endChar + " ";
                    //Append to joinedmessage text
                    joinedMessageText+= msg;
                }
                
                //Copy over message
                messageText = joinedMessageText;

                //Remove space inside end and start of qoutes
                messageText = messageText.replace(/\" ([^\"]*) \"/g, "\"$1\"");

                //Remoce space between .. and ... needs to be done twice to get ...
                messageText = messageText.replace(/\. \./g,"..");
                messageText = messageText.replace(/\. \./g,"..");
                
                //Capitalise all the i
                messageText = messageText.replace(/ i([ '.!?])/g," I$1");

                //Add command back on if any
                messageText = cmd+messageText;
                lastUrl = 0;
                
                while((lastUrl != messageText.length) && (messageText.indexOf('http://',lastUrl)!= -1))
                {
                    indexStart = messageText.indexOf('http://',lastUrl);
                    indexEnd = messageText.indexOf(']',indexStart);

                    before = messageText.slice(0,indexStart);
                    url =  messageText.slice(indexStart,indexEnd);
                    url = url.replace(/\. /g,".");
                    after = messageText.slice(indexEnd,messageText.length);
                    messageText = before + url + after;
                    lastUrl = indexEnd;
                }
            }
...
ajaxChat.handleOnlineUsers = function(userNodes) {
    ...
    if(userNodes.length) {
        ...
        for(var i=0; i<userNodes.length; i++) {
            ...
            if (userNodes[i].getAttribute('status') == 'afk') {
              groupClass = groupClass + ' away'; 
            }
            ...
            this.onlineUsers[i] = this.decodeSpecialChars(userName);
...
ajaxChat.summonStaff = function(Staff) {
	
    var answer = confirm(("Are you sure you want to summon a "+ Staff + " to come to this room?\n\nYou will be unable to chat for about one minute (to prevent abuse of this feature)."));
    if (answer) {
	switch(Staff)
	{
		case('admin'): this.sendMessage('/summonAdmin'); break;
		case('mod'): this.sendMessage('/summonModerator'); break;
		case('mentor'): this.sendMessage('/summonMentor'); break;
		case('coder'): this.sendMessage('/summonCoder'); break;
	}
    }
}
