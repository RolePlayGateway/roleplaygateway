...
if ($user->data['user_id'] >= 4) {
  ...
  WHERE
    /* List all user-owned places */
    (owner = '.(int) $user->data['user_id'] .' AND id > 0)
    OR (roleplay_id IN (SELECT roleplay_id FROM rpg_permissions WHERE isCoGM = 1 AND user_id = '.(int) $user->data['user_id'].'))
    /* List all places in universes owned by the user */
    OR (roleplay_id IN (SELECT id FROM rpg_roleplays WHERE owner = '.(int) $user->data['user_id'].'))
    /* Locations at which the user owns a player/character */
    OR (id IN (SELECT location FROM rpg_characters WHERE owner = '.(int) $user->data['user_id'].'))
