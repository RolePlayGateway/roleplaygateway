...
if (empty($characterID)) {
	...
	$sql = 'SELECT id, name, owner, url FROM rpg_characters WHERE url = "'.$db->sql_escape($characterURL).'" AND roleplay_id = '.(int) $roleplay['id'];
	$characterResult = $db->sql_query($sql);
	$character = $db->sql_fetchrow($characterResult);
	$db->sql_freeresult($characterResult);
} else {
	$sql = 'SELECT id, name, owner, url FROM rpg_characters WHERE id = '.(int) $characterID;
	$characterResult = $db->sql_query($sql);
	$character = $db->sql_fetchrow($characterResult);
	$db->sql_freeresult($characterResult);
}
...
$sql = 'INSERT INTO rpg_characters_followed (`character_id`, `user_id`) VALUES ('.(int) $character['id'].', '.$user->data['user_id'].')';
$db->sql_query($sql);

if ($character['owner'] !== $user->data['user_id']) {
	award_medal_from(0, 36, $character['owner'], $user->data['username'] . ' followed your character, '. $character['name'] .'!', time());
}

header('Content-Type: application/json');
echo(json_encode(array(
	'status' => 'success',
	'message' => 'Character successfully followed!'
)));
