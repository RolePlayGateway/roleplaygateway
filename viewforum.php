...
//$prs_data =& prs_fetch_votes_topics($topic_list, $rowset);
...
if ($forum_id == 4) {
	...
	$news_result = $db->sql_query("SELECT post_id,post_subject,post_time FROM gateway_posts WHERE topic_id = 10572 ORDER BY post_time DESC");
...
if (sizeof($topic_list))
{
	...
	foreach ($topic_list as $topic_id)
	{
		...
		// Custom roleplay stuff
		$sql = 'SELECT id FROM rpg_content WHERE old_post_id = '.(int) $row['topic_first_post_id'].'';
		$result = $db->sql_query($sql);
		...
		$template->assign_block_vars('topicrow', array(
			...
			// BEGIN ROLEPLAY STUFF
			'S_IS_ROLEPLAY'		=> ($roleplay_data) ? true : false,
			// END ROLEPLAY STUFF	
		...
		// PRS
		// culprit is prs_display_rating_topics
		
		// $topic_stars = prs_stars(@$prs_data[$topic_id]['score']);
		
		// $template->alter_block_array('topicrow',array('PRS_STARS' => $topic_stars['PRS_STARS']),array('TOPIC_ID' => $topic_id),'change');
