    ...
    var svgContainer,
        ...
        nodeBuilder = function(node){
            ...
            if (typeof node.data != 'undefined') {
              node.data.permalink = 'http://www.roleplaygateway.com/roleplay/'+node.data.roleplayURL+'/places/'+node.data.url ;
            } else {
              console.log('ERROR');
              console.log(node)
            }
            ...
            /* stuff.append(
              Viva.Graph.svg('image')
                .attr('width', 50)
                .attr('height', 50)
                .attr('fill', '#00a2e8').text(node.data.name).link(node.data.permalink + '/image.png')
            ); */
            
            var container = Viva.Graph.svg('svg');
