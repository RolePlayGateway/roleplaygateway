...
function get_folder($user_id, $folder_id = false)
{
	...
	foreach ($folder as $f_id => $folder_ary)
	{
		...
		$template->assign_block_vars('folder', array(
			...
			'S_INVITE_TO_WELCOME_DESK'	=> (($user->data['user_posts'] == 0) || ($user->data['user_id'] == 4)) ? true : false)
		);
	}
...
function submit_pm($mode, $subject, &$data, $put_in_outbox = true)
{
	...
	if (preg_match("/roleplay/i",$user->data['username'])) {
		trigger_error('You are not permitted to send PMs at this time.');
	}

	$spam_text = "RoleplayGateway seems pretty cool I guess, I plan to post here plenty...thought I'd refer you to a nice roleplaying forum that just started up with a lot of favorite RP genres and types to choose from, original fantasy, modern, anime, sci-fi, shows/movies/books, etc., anything and everything, we've got it, or just pursue your dreams and make your own roleplay project there. It's a lot of fun and growing steadily, join the movement before it's too late! ^_^
Anyway, hope to see you both here and there, I guarantee you'll enjoy it if you give it a chance.
Check it out:
http://doiop.com/roleplayfun";
	$spam_text2 = "Just wanted to say hi and give you a friendly invitation to a fun forum if you like roleplay.  It's got a lot of favourite RP genres and types to choose from, original fantasy, modern, anime, sci-fi, shows/movies/books, etc., anything and everything, we've got it, or you can just make your own RP. It's a lot of fun. ^_^
http://doiop.com/roleplayfun";

	if ($user->data['user_posts'] < 5) {

		similar_text($spam_text,$data['message'],$percent);
		similar_text($spam_text2,$data['message'],$percent2);

		// Array ( [u] => Array ( [4] => to ) ) 
		if (($percent >= 70) || ($percent2 >= 70) || preg_match("/doiop.com\/roleplayfun/i", $data['message'])) {

			try {
				$url = "http://www.stopforumspam.com/add/";
				$ch = curl_init();

				$data = array(
					'username' => $user->data['username'],
					'email' => $user->data['user_email'],
					'ip_addr' => $user->data['session_ip'],
				...
				if (curl_exec ($ch)) {
					trigger_error('You cannot submit private messages at this time.');
				}
			} catch (Exception $e) {

			}

			user_ban("user",array($user->data['username'], $user->data['user_ip']), 0, null, 0, "Spammer.", "Spammer.");
	...
	if ($mode != 'edit')
	{
		...
		//Check if user flagged as spammer if so dont inform recipients.
		$sql = "SELECT user_spammer FROM gateway_users WHERE user_id = " . (int) $data['from_user_id'];
		$result = $db->sql_query($sql);
		$row = $db->sql_fetchrow($result);
		$spammer = $row['user_spammer'];
		if (!$spammer)
		{
