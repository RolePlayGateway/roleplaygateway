...
function generate_text_for_display($text, $uid, $bitfield, $flags)
{
	...
	$text = preg_replace_callback('/\[place:(\d*)\]/', 'getPlaceHTMLforID', $text);
	$text = preg_replace_callback('/\[group:(\d*)\]/', 'getGroupHTMLforID', $text);
	$text = preg_replace_callback('/\[place:(.*)\]/U', 'getPlaceHTMLforName', $text);
	$text = preg_replace_callback('/\[group:(.*)\]/U', 'getGroupHTMLforName', $text);

	return $text;
}

function getPlaceHTMLforID($matches) {
	global $db, $user;

	$sql = 'SELECT id, name, url, synopsis, roleplay_id FROM rpg_places WHERE id = '. (int) $matches[1];
	$result = $db->sql_query($sql);
	$place = $db->sql_fetchrow($result);
	$db->sql_freeresult($result);

	$sql = 'SELECT id, title, url, description FROM rpg_roleplays WHERE id = '. (int) $place['roleplay_id'];
	$result = $db->sql_query($sql);
	...
	if (!empty($place)) {
		...
		return '<div class="thing-icon">
			  			<a href="/roleplay/'.$roleplay['url'].'/places/'.$place['url'].'/">
			  				<img src="/roleplay/'.$roleplay['url'].'/places/'.$place['url'].'/image" class="crisp-border">
			  				<h4>'.$place['name'].'</h4>
			  				<p>'.$place['synopsis'].'</p>
			  			</a>
			  		</div>';
...
function getPlaceHTMLforName($matches) {
	global $db, $user;

	$sql = 'SELECT id, name, url, synopsis, roleplay_id FROM rpg_places WHERE name = "'. $db->sql_escape($matches[1]).'"';
	$result = $db->sql_query($sql);
	$place = $db->sql_fetchrow($result);
	$db->sql_freeresult($result);
...
function getGroupHTMLforID($matches) {
	global $db, $user;

	$sql = 'SELECT id, name, slug, synopsis, roleplay_id FROM rpg_groups WHERE id = '. (int) $matches[1];
	$result = $db->sql_query($sql);
	$group = $db->sql_fetchrow($result);
	$db->sql_freeresult($result);

	$sql = 'SELECT id, title, url, description FROM rpg_roleplays WHERE id = '. (int) $group['roleplay_id'];
	$result = $db->sql_query($sql);
	$roleplay = $db->sql_fetchrow($result);
	$db->sql_freeresult($result);
	...
	if (!empty($group)) {
		...
		return '<div class="thing-icon">
			  			<a href="/roleplay/'.$roleplay['url'].'/groups/'.$group['slug'].'">
			  				<img src="/roleplay/'.$roleplay['url'].'/groups/'.$group['slug'].'/logo" class="crisp-border">
			  				<h4>'.$group['name'].'</h4>
			  				<p>'.$group['synopsis'].'</p>
			  			</a>
			  		</div>';
...
function getGroupHTMLforName($matches) {
	global $db, $user;

	$sql = 'SELECT id, name, slug, synopsis, roleplay_id FROM rpg_groups WHERE name = "'. $db->sql_escape($matches[1]).'"';
	$result = $db->sql_query($sql);
	$group = $db->sql_fetchrow($result);
	$db->sql_freeresult($result);
