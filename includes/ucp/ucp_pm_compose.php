...
function compose_pm($id, $mode, $action)
{
	...
	if ((!$config['allow_mass_pm'] || !$auth->acl_get('u_masspm')) && !empty($address_list['u']) && $max_recipients && sizeof($address_list['u']) > $max_recipients)
	{
	...
	if ($add_to || $add_bcc)
	{
		...
		if (sizeof($group_list))
		{
			foreach ($group_list as $group_id)
			{
				...
				if ($group_id == 13) {
					if (!function_exists('getSeniorStaff')) {
						include_once('../includes/functions.php');
					}
					$user_id_ary = getSeniorStaff();
				}
		...
		// User ID's to add...
		$user_id_ary = (sizeof($user_id_ary)) ? $user_id_ary  : array();
