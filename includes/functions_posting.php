...
function submit_post($mode, $subject, $username, $topic_type, &$poll, &$data, $update_message = true, $update_search_index = true)
{
	...
	if ($post_mode == 'post' || $post_mode == 'reply')
	{
		...
		if (in_array($user->data['user_id'],getChatOnlineUserIDs()) && (!in_array($data['forum_id'],$private_forums))) {
		
			$message = $user->data['username']." wrote a new post on the forum: [url=http://www.roleplaygateway.com/post".$data['post_id'].".html#p".$data['post_id']."]". $subject ."[/url]";
		
			$sql = "INSERT INTO ajax_chat_messages (`userId`,`userRole`,`ip`,`dateTime`,`userName`,`channel`,`text`) VALUES ('2147483647','4','127.0.0.1',NOW(),'Forum Announcer','0','".$db->sql_escape($message)."')";
			
			$result = $db->sql_query($sql);
			
		} 
		// end Custom HTTP chat announcement
		*/
