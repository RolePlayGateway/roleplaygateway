...
class mcp_queue
{
	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'unreviewed_roleplays':
				$this->tpl_name = 'mcp_roleplays';
				$user->add_lang(array('posting', 'viewtopic'));
				
				$total_roleplays = 0;
				$sql = "SELECT count(*) as count FROM rpg_roleplays WHERE status='Pending Review'";
				$result = $db->sql_query($sql);
				$row = $db->sql_fetchrow($result);
				$total_roleplays = $row['count'];

				$sql = "SELECT id, title, description, url, owner FROM rpg_roleplays WHERE status='Pending Review'";
				...
				$template->assign_vars(array(
						'L_TITLE' =>'Rolplays awaiting review and completion.',
						'L_EXPLAIN' =>'This is a list of all roleplays where the owner has said they are completed.',
						'REVIEW'	=> true,
						'PAGINATION'	=> generate_pagination($this->u_action, $total_roleplays, $config['topics_per_page'], $start),
                                        	'PAGE_NUMBER'	=> on_page($total_roleplays, $config['topics_per_page'], $start)
						));


			break;
			case 'unclosed_roleplays':
				$this->tpl_name = 'mcp_roleplays';
				$user->add_lang(array('posting', 'viewtopic'));
				
				$total_roleplays = 0;
				$sql = "SELECT count(*) as count FROM rpg_roleplays WHERE status='Pending Closure'";
				$result = $db->sql_query($sql);
				$row = $db->sql_fetchrow($result);
				$total_roleplays = $row['count'];

				$sql = "SELECT id, title, description, url, owner FROM rpg_roleplays WHERE status='Pending Closure'";
				$result = $db->sql_query_limit($sql, $config['topics_per_page'], $start);
				while ($row = $db->sql_fetchrow($result))
				{
					$template->assign_block_vars('postrow', array(
				...
				$template->assign_vars(array(
						'L_TITLE' =>'Rolplays awaiting closure',
						'L_EXPLAIN' =>'This is a list of all roleplays where the owner has requested they are closed.',
						'CLOSE'	=> true,
						'PAGINATION'	=> generate_pagination($this->u_action, $total_roleplays, $config['topics_per_page'], $start),
                                        	'PAGE_NUMBER'	=> on_page($total_roleplays, $config['topics_per_page'], $start)
						));
