...
class acp_users
{
	...
	function user_merge($new_user, $old_user)
	{
		global $user, $db;
	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'overview':
				...
				$merge			= request_var('merge', 0);
				$merge_user		= request_var('merge_user', '');
				$delete_type	= request_var('delete_type', '');
				$ip				= request_var('ip', 'ip');
		
				$template->assign_vars(array(
					'L_MERGE_USER' =>'Merge users',
					'L_MERGE_USER_EXPLAIN' => 'Enter the username of the account you wish to merge with this one.'
				));
				...
				if ($submit)
				{
					...
					if ($merge && $merge_user != '')
					{
						$sql="SELECT user_id FROM gateway_users WHERE username = '".$merge_user ."'";
						$result = $db->sql_query($sql);
						$row = $db->sql_fetchrow($result);
						$merge_user_id=$row['user_id'];

						if ($merge_user_id =='')
						{
							trigger_error("User ".$merge_user." not found.");
						}
						$db->sql_freeresult($result);

						if (confirm_box(true))
						{
							$this->user_merge($user_id, $merge_user_id);
							trigger_error('Users merged' . adm_back_link($this->u_action));
						}
						else
						{
					...
					$data = array(
						...
						'user_spammer'		=> request_var('user_spammer','0'),
					);
					...
					$update_spammer = ($data['user_spammer'] != $user_row['user_spammer']) ? $data['user_spammer'] : false;
					...
					if (!sizeof($error))
					{
						...
						if ($update_spammer !== false)
						{
							$sql_ary += array(
								'user_spammer'		=> $update_spammer,
							);

							add_log('user', $user_id, 'LOG_USER_UPDATE_SPAMMER', $user_row['username'], $user_row['user_spammer'], $update_spammer);
						}
				...
				$template->assign_vars(array(
					...
					'L_USER_SPAMMER'	=> 'Spammer',
					'USER_SPAMMER'		=> $user_row['user_spammer'],
				));
