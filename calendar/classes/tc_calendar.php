...
class tc_calendar{
	...
	function writeScript(){
		...
		$line = $this->writeHidden();

		//check whether it is a date picker
		if($this->date_picker){
			$line = $line."<div style=\"position: relative; z-index: ".$this->zindex."; float: left;\">";
			
			if($this->show_input){
				$line = $line.$this->writeDay();
				$line = $line.$this->writeMonth();
				$line = $line.$this->writeYear();
			}else{
				$line = $line." <a href=\"javascript:toggleCalendar('".$this->objname."');\">";
				$line = $line.$this->writeDateContainer();
				$line = $line."</a>";
			}			
			
			$line = $line." <a href=\"javascript:toggleCalendar('".$this->objname."');\">";						
//			if(is_file($this->icon)){
//				$line = $line."<img src=\"".$this->icon."\" id=\"tcbtn_".$this->objname."\" name=\"tcbtn_".$this->objname."\" border=\"0\" align=\"absmiddle\" />";
//			}else $line = $line.$this->txt;				
			$line = $line."</a>";
			
			$line = $line.$this->writeCalendarContainer();
			
			return $line."</div>";
		}else{
			return $line.$this->writeCalendarContainer();
		}			
	}
	...
	function writeCalendarContainer(){
        ...
        $line = "";
		$line = $line."<div id=\"div_".$this->objname."\" style=\"position:".$div_position.";visibility:".$div_display.";z-index:100;".$div_align."\" class=\"div_calendar calendar-border\">";
		$line = $line."<IFRAME id=\"".$this->objname."_frame\" src=\"".$this->path."calendar_form.php".$paramStr."\" frameBorder=\"0\" scrolling=\"no\" allowtransparency=\"true\" width=\"100%\" height=\"100%\" style=\"z-index: 100;\"></IFRAME>";
	...
	function writeDay(){
		...
		$line = $line."<select name=\"".$this->objname."_day\" id=\"".$this->objname."_day\" onChange=\"javascript:tc_setDay('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcday\">";
		$line = $line."<option value=\"00\">Day</option>";
		for($i=1; $i<=$total_days; $i++){
			$selected = ((int)$this->day == $i) ? " selected" : "";
			$line = $line."<option value=\"".str_pad($i, 2 , "0", STR_PAD_LEFT)."\"".$selected.">".$i."</option>";
		}
		$line = $line."</select> ";
        return $line;
	}
	...
	function writeMonth(){
		$line = $line."<select name=\"".$this->objname."_month\" id=\"".$this->objname."_month\" onChange=\"javascript:tc_setMonth('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcmonth\">";
		$line = $line."<option value=\"00\">Month</option>";
		
		$monthnames = $this->getMonthNames();		
		for($i=1; $i<=sizeof($monthnames); $i++){
			$selected = ((int)$this->month == $i) ? " selected" : "";
			$line = $line."<option value=\"".str_pad($i, 2, "0", STR_PAD_LEFT)."\"".$selected.">".$monthnames[$i-1]."</option>";
		}
		$line = $line."</select> ";
        return $line;
	}
	...
	function writeYear(){
		//$line = $line."<input type=\"textbox\" name=\"".$this->objname."_year\" id=\"".$this->objname."_year\" value=\"$this->year\" maxlength=4 size=5 onBlur=\"javascript:tc_setYear('".$this->objname."', this.value, '$this->path');\" onKeyPress=\"javascript:if(yearEnter(event)){ tc_setYear('".$this->objname."', this.value, '$this->path'); return false; }\"> ";
		$line = $line."<select name=\"".$this->objname."_year\" id=\"".$this->objname."_year\" onChange=\"javascript:tc_setYear('".$this->objname."', this[this.selectedIndex].value);\" class=\"tcyear\">";
		$line = $line."<option value=\"0000\">Year</option>";
		...
		for($i=$year_end; $i>=$year_start; $i--){
			...
			$line = $line."<option value=\"".$i."\"".$selected.">".$i."</option>";
		}
		$line = $line."</select> ";
        return $line;
	}
	...
	function eHidden($suffix, $value) {
		...
		$line = "<input type=\"hidden\" name=\"".$this->objname.$suffix."\" id=\"".$this->objname.$suffix."\" value=\"".$value."\" />";
        return $line;
	}
	...
	function writeHidden(){
        $line="";
		$line .= $this->eHidden('', $this->getDate());
		$line .= $this->eHidden('dp', $this->date_picker);
		$line .= $this->eHidden('year_start', $this->year_start);
		$line .= $this->eHidden('year_end', $this->year_end);
		$line .= $this->eHidden('mon', $this->startMonday);
		$line .= $this->eHidden('da1', $this->time_allow1);
		$line .= $this->eHidden('da2', $this->time_allow2);
		$line .= $this->eHidden('sna', $this->show_not_allow);
		$line .= $this->eHidden('aut', $this->auto_submit);
		$line .= $this->eHidden('frm', $this->form_container);
		$line .= $this->eHidden('tar', $this->target_url);
		$line .= $this->eHidden('inp', $this->show_input);
		$line .= $this->eHidden('fmt', $this->date_format);
		$line .= $this->eHidden('dis', implode(",", $this->dsb_days));
		$line .= $this->eHidden('pr1', $this->date_pair1);
		$line .= $this->eHidden('pr2', $this->date_pair2);
		$line .= $this->eHidden('prv', $this->date_pair_value);
		$line .= $this->eHidden('pth', $this->path);
		
		$line .= $this->eHidden('spd', $this->check_json_encode($this->sp_dates));
		$line .= $this->eHidden('spt', $this->sp_type);
		$line .= $this->eHidden('spr', $this->sp_recursive);
        return $line;
	}
	...
	function writeDateContainer(){
		...
		return "<span id=\"divCalendar_".$this->objname."_lbl\" class=\"date-tccontainer\">".$dd."</span>";
	}
